...
export default function Watchlist({ userId }: { userId: string }) {
  ...
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
    {movies.map((movie) => (
      ...
      >
        <div className="rounded-lg overflow-hidden hover:scale-105 transition-transform w-full">
          <img
            src={`https://image.tmdb.org/t/p/w200${movie.poster_path}`}
            alt={movie.movie_title}
            className="w-full h-[240px] object-cover"
          />
          <div className="bg-gray-700 text-white text-sm font-medium px-2 py-1 w-full h-[40px] truncate overflow-hidden whitespace-nowrap rounded-b-md">
            {movie.movie_title}
          </div>
        </div>
      </Link>
