...
export default function Comment({ userId }: { userId: string }) {
  ...
  return (
    <div className="mt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mt-4 flex justify-between items-center">
        {Comments.map((comment) => (
          ...
          >
            ...
            <div className="flex flex-col justify-between flex-1 pl-4 min-w-0">
              <div>
                <p className="text-base text-xl font-semibold">{comment.movie_title}</p>

                {/* Comentario con límite de 3 líneas */}
                <p className="mt-2 text-sm text-gray-300 break-words w-full line-clamp-3">
                  {comment.comentario}
                </p>
              </div>
               {/* Botón "Ver más" */}
                <button
                  onClick={(e) => {
                    e.preventDefault(); // para evitar navegación en el Link
                    openModal(comment);
                  }}
