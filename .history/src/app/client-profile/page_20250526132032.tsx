...
export default function ProfilePage() {
  ...
  try {
    console.log("Llamando a:", `/api/seguidos/${userId}`); // 💡 Asegura que la ruta esté bien formada

    const res = await fetch(`/api/seguidos/${userId}`);
    if (!res.ok) throw new Error("Failed to fetch following count");

    const data = await res.json();
    console.log("Respuesta del servidor:", data); // 💡 Verifica la estructura
      ...
      <div className="max-w-4xl mx-auto mt-10 space-y-6">
        ...
        <div className="flex items-center justify-between border rounded-xl shadow-md p-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-col ml-25 items-center">
