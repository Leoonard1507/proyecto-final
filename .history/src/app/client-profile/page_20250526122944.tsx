...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (session?.user?.id) {
      setUserId(session.user.id);
      fetchUserData(session.user.id);
      fetchFollowingCount(session.user.id); // <-- nueva llamada
    }
  }, [session]);

  // Ya tienes fetchUserData...

  // Nueva función para obtener el número de gente que sigue el usuario
  const fetchFollowingCount = async (userId: string) => {
    try {
      const res = await fetch(`/api/infoPerfil/${userId}`);
      if (!res.ok) throw new Error("Failed to fetch following count");
      const data = await res.json();
      // Tu API devuelve un array con un objeto { following_count: number }
        ...
        <div className="flex items-center justify-between border rounded-xl shadow-md p-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-col">
              <span className="text-base font-semibold">{followingCount !== null ? followingCount : '...'}</span>
